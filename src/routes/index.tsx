...
function HomePage() {
  ...
  const heroTitle = banner?.title ?? "Discover Premium Timepieces";
  const heroSubtitle =
    banner?.subtitle ??
    "Curated luxury watches for every occasion. Engineered for those who measure life in moments, not minutes.";
  const heroEyebrow = banner?.eyebrow ?? "Est. 2025 — Swiss Crafted";
  const heroImage = banner?.image || heroWatch;
  const ctaLabel = banner?.cta_label ?? "Shop Now";
  const ctaLink = banner?.cta_link ?? "/collections";

  return (
    <>
      {/* Hero — driven by `banners` table */}
          ...
          >
            ...
            <h1 className="mt-4 font-display text-5xl leading-tight md:text-6xl lg:text-7xl">
              {heroTitle.split(" ").map((word, i, arr) =>
                i === arr.length - 1 ? (
                  <span key={i} className="italic text-[var(--gold)]">
                    {" "}
                    {word}
                  </span>
                ) : (
        ...
        <div className="mx-auto grid max-w-[1200px] gap-12 px-6 md:grid-cols-2 md:px-8">
          <div>
            ...
            <p className="mt-6 text-white/70">
              Time & Trend was founded on the belief that a beautiful timepiece is more than
              just an accessory—it's a statement. Every piece we offer is a testament to both
